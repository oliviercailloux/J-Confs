...
Durant ce projet nous avons rencontré des difficultés dans la compréhension du code existant. En effet le code était mal organisé (absence de packages ) et parfois incompréhensible dans certaines méthodes qui étaient mal documentées. De plus, cela nous a pénalisé car un grand nombre des consignes à respecter au sein des PR n’était pas respecté au sein de la branche master initial.

La branche comportait des centaines de message d’avertissements, du code non formaté et mal documenté (Javadoc non explicite). Ainsi lors des PR, un grand nombre des remarques que nous recevions de votre part faisaient référence à du code réalisé par l’ancien groupe. Nous avons donc consacré un temps conséquent afin de corriger le code existant entraînant ainsi un retard dans l’implémentation de nouvelles fonctionnalités.
...
Pour le stockage de conférence en ligne nous avons dû faire face à une absence de documentation pour la mise en place de cette fonctionnalité (le service en ligne fruux reconnaît même que sa documentation pour développeurs est assez limitée).

Cela a impacté notre planning prévisionnel car nous avons initialement prévu d’implémenter la fonctionnalité d’ajout et d’édition d’une conférence en une seule itération (3). Mais certaines méthodes présentes au sein de la librairie caldav4j ne fonctionnaient pas sur la version stable de la librairie (ajout et modification d’évènements) . Cette version stable était par ailleurs préconisé au sein du répertoire github officiel de la librairie. Nous avons donc consacré un temps important pour comprendre que le problème venait de la version préconisée qui rencontrait des dysfonctionnements (voir https://github.com/caldav4j/caldav4j/blob/master/README.md ).
