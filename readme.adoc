...
I particularly worked on conference and calendar packages for two iterations in a row. Which implies that these are the packages that I know best. I had the opportunity to improve what the previous developers had already implemented. I also could set up J-Unit tests for some classes, which allowed me on the one hand to check that they worked well and on the other hand, it gave me ideas of better implementations.	
