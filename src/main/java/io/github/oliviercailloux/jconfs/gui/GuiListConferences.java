...
public class GuiListConferences {
	...
	public void editConference(@SuppressWarnings("unused") Event e) throws Exception {
		if (isAllFieldsValid()) {
			addConference(!(Strings.isNullOrEmpty(txtUrl.getText())), !(Strings.isNullOrEmpty(txtRegisFee.getText())));
	 ...
	 * @throws CalDAV4JException
	 */
	public void addConference(boolean url, boolean fees) throws MalformedURLException, CalDAV4JException {
		...
		theBuild = theBuild.setTitle(txtTitle.getText())
				...
				.setEndDate(localDateEnd.atStartOfDay(ZoneOffset.UTC).toInstant()).setCity(txtCity.getText())
