...
/**
 * this class provides method to made GUI for displaying map inspired from awt sample from
 * org.mapsforge.samples.awt
 * 
 * @author stanislas
 *
 */
public class MapGUI {
  ...
  public MapGUI(String mapName, Display display) throws NullPointerException, IOException {
    ...
    this.startPoint = new FixedPixelCircle(new LatLong(48.866667, 2.333333), 5, paintFill, null);// Paris
                                                                                                 // by
                                                                                                 // default
    ...
    // endPoint cration
    this.endPoint = new FixedPixelCircle(new LatLong(48.866667, 2.333333), 5, paintFill, null);// Paris
                                                                                               // by
                                                                                               // default
    startPoint.setVisible(true);
  ...
  private BoundingBox addDownloadLayers(MapView mapView) {
    ...
    TileCache tileCache =
        AwtUtil.createTileCache(tileSize, mapView.getModel().frameBufferModel.getOverdrawFactor(),
            1024, new File(System.getProperty("java.io.tmpdir"), UUID.randomUUID().toString()));

    mapView.getModel().displayModel.setFixedTileSize(tileSize);
    TileDownloadLayer tileDownloadLayer =
        createTileDownloadLayer(tileCache, mapView.getModel().mapViewPosition, tileSource);
   ...
   */
  public MapGUI(String mapName, Display display, LatLong endPoint)
  ...
  /**
   * this method create a mapTile from mapFile data and add it to the mapView if the file doesn't
   * exist,this method ask an URL to the user for download the map If Local==false it display an
   * online map
   * 
  ...
  private BoundingBox addLayers(MapView mapView, String mapName) throws IOException {
    ...
    TileRendererLayer tileRendererLayer =
        createTileRendererLayer(tileCache, mapDataStore, mapView.getModel().mapViewPosition, null);
   ...
   */
  private TileDownloadLayer createTileDownloadLayer(TileCache tileCache,
